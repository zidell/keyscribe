...
KeyScribe는 사용자가 녹음한 음성을 설정에서 선택한 OpenAI 또는 ElevenLabs의 음성 인식 API로 전송합니다. 모델 목록을 가져올 때도 선택한 서비스의 API에 요청합니다. 각 서비스의 데이터 처리와 비용은 해당 서비스의 정책 및 사용자 계정 설정을 따릅니다.
