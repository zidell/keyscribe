...
macOS Swift/AppKit 실행 파일의 첫 릴리스 빌드는 `bash native/macos/build.sh`로 생성된다. 메뉴 막대, 사용자 설정 이관, 마이크 녹음, 화면 우측 하단의 녹음·변환 상태 오버레이와 실시간 음량 막대, 단축키의 누르기/토글 방식, Esc 취소, OpenAI/ElevenLabs 전사, 클립보드 붙여넣기와 자동 Enter를 구현했다. 네트워크 취소와 늦게 도착한 응답을 구분한다.
